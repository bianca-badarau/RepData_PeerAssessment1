...
So the mean of the total number of steps per day is **`r step_mean`**, while the median is equal to **`r step_median`**.
...
The interval with the highest step average is **`r intvl_max_steps`**.
...
The new mean is **`r step_mean_noNAs`**, while the median is **`r step_median_noNAs`**. We can see that the mean is almost unchanged, while the median has increased slightly.
...
```{r plot4, fig.height = 7, fig.width = 8}
